...
class ErrorReportingClient {
    ...
    func trackError(methodName: String, error: Error, userId: String? = nil) async {
        do {
            let payload = createEnhancedErrorPayload(methodName: methodName, error: error, userId: userId)

            try await sendEnhancedErrorToAPI(payload: payload)
            logger.info("Enhanced error reported successfully for method: \(methodName)")
        } catch {
    ...
    /// Create enhanced error payload with structured context
    private func createEnhancedErrorPayload(methodName: String, error: Error, userId: String?) -> EnhancedErrorPayload {
        if let paraError = error as? ParaError {
            createStructuredErrorPayload(methodName: methodName, paraError: paraError, userId: userId)
        } else {
            // Fallback to legacy error payload
            EnhancedErrorPayload(
                ...
                userId: userId,
                // Enhanced fields (empty for legacy errors)
                errorCode: nil,
                userMessage: nil,
                isRetryable: nil,
                context: nil,
    ...
    /// Create structured error payload for ParaError types
    private func createStructuredErrorPayload(methodName: String, paraError: ParaError, userId: String?) -> EnhancedErrorPayload {
        EnhancedErrorPayload(
            methodName: methodName,
            error: ErrorInfo(
                name: String(describing: type(of: paraError)),
                message: paraError.description,
            ),
            sdkType: "SWIFT",
            sdkVersion: ParaPackage.version,
            environment: environment,
            ...
            userId: userId,
            // Enhanced fields from structured ParaError
            errorCode: paraError.errorCode,
            userMessage: paraError.userMessage,
            isRetryable: paraError.isRetryable,
            context: paraError.context,
        )
    }
    ...
    /// Send enhanced error payload to the API
    private func sendEnhancedErrorToAPI(payload: EnhancedErrorPayload) async throws {
        ...
        // Use custom encoder for better structured error encoding
        ...
        if let context = payload.context {
            logger.debug("Sending structured error - Operation: \(context.operation), Chain: \(context.chainId ?? "none"), Code: \(payload.errorCode ?? "none")")
        }
        ...
        do {
            ...
            logger.error("Failed to send enhanced error report: \(error.localizedDescription)")
            throw error
        }
    }
}
...
/// Enhanced error payload with structured context
private struct EnhancedErrorPayload: Codable {
    ...
    // Enhanced fields from ParaBridgeError
...
/// Legacy error payload for backward compatibility
